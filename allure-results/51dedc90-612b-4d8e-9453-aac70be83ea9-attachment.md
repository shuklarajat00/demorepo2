...
```ts
  ...
  5  | const username = process.env.username as string; // Access the username from environment variables
  6  | const password = process.env.password as string; // Access the password from environment variables
  7  | const url = process.env.url as string; // Access the URL from environment variables
  8  | const To_check_dotenv = process.env.To_check_dotenv as string; // Access the To_check_dotenv variable from environment variables
  9  | 
